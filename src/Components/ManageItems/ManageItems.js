...
const ManageItems = () => {
  ...
  const handleDeliver = (products) => {
    ...
    fetch("https://intense-meadow-16226.herokuapp.com/myitems", {
      method: "POST",
      body: JSON.stringify({
        name: name,
        price: price,
  ...
  const handleDelete = (id) => {
    console.log(id);
    fetch(`https://intense-meadow-16226.herokuapp.com/additem/${id}`, {
      method: "DELETE",
    })
      .then((response) => response.json())
      .then((data) => {
  ...
  <Card.Body>
    ...
    <Button variant="danger" onClick={()=>handleDelete(product._id)}>Delete</Button>
      ...
      <div className="row text-center">
        <Table striped bordered hover>
          ...
          <tbody>
            {products.map((product) => (
              <tr className=" container col-md-12 col-md-6 col-lg-4">
                <td>{product._id}</td>
                <td>{product.name}</td>
                <td>{product.price}</td>
        ...
        <Button
          className="text-center btn btn-secondary w-25 mx-auto text-white "
